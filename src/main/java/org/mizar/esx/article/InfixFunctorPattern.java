...
public class InfixFunctorPattern extends EsxElement implements PatternInterface {
    ...
    @Override
    public void process() {
        ESX_Processor.actions.actionInfixFunctorPatternBeforeLeftLoci(this);
        setLeftLoci((Loci) EsxElementFactory.create(getElement().elements().get(0)));
        ESX_Processor.actions.actionInfixFunctorPatternAfterLeftLoci(this);

        ESX_Processor.actions.actionInfixFunctorPatternBeforeRightLoci(this);
        setRightLoci((Loci) EsxElementFactory.create(getElement().elements().get(1)));
        ESX_Processor.actions.actionInfixFunctorPatternAfterRightLoci(this);
    }
}
