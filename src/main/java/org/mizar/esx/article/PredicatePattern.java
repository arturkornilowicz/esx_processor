...
public class PredicatePattern extends EsxElement implements PatternInterface {
    ...
    @Override
    public void process() {
        ESX_Processor.actions.actionPredicatePatternBeforeLeftLoci(this);
        setLeftLoci((Loci) EsxElementFactory.create(getElement().elements().get(0)));
        ESX_Processor.actions.actionPredicatePatternAfterLeftLoci(this);

        ESX_Processor.actions.actionPredicatePatternBeforeRightLoci(this);
        setRightLoci((Loci) EsxElementFactory.create(getElement().elements().get(1)));
        ESX_Processor.actions.actionPredicatePatternAfterRightLoci(this);
    }
}
